...
namespace Microsoft.Azure.Batch.SoftwareEntitlement.Common.Tests
{
    public abstract class ErrorableTests
    {
        ...
        public class OnSuccess
        {
            ...
            {
                var result = _success.OnSuccess(num => Errorable.Success(num + 1));
                result.GetValue().Should().Be(43);
            }

            [Fact]
            public void SuccessThenFailure_HasExpectedErrors()
            {
                var result = _success.OnSuccess(num => Errorable.Failure<int>("expected error"));
                result.GetErrors().Count().Should().Be(1);
                result.GetErrors().Should().Contain("expected error");
            }
            ...
            {
                var executed = false;
                var result = _failure.OnSuccess(num =>
                {
                    executed = true;
